...
## Follow-up fix
- Hardened button state synchronization by explicitly reflecting both the `disabled` property and `disabled` attribute via `setProcessButtonState` to avoid stale disabled UI state.
- Added targeted regression tests for state reflection and full upload change-event transitions (valid file enables button, invalid file disables it).
