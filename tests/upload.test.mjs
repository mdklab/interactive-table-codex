...
import { buildCsvFile } from "./fixtures.mjs";

test("validateCsvFile accepts csv by extension", () => {
  const result = validateCsvFile(
    buildCsvFile({ name: "people.csv", type: "application/octet-stream", size: 32 })
  );
...
test("validateCsvFile rejects non-csv file types", () => {
  const result = validateCsvFile(buildCsvFile({ name: "people.json", type: "application/json", size: 12 }));
...
test("validateCsvFile rejects text/plain when extension is not csv", () => {
  const result = validateCsvFile(buildCsvFile({ name: "notes.txt", type: "text/plain", size: 18 }));
...
test("validateCsvFile rejects empty csv files", () => {
  const result = validateCsvFile(buildCsvFile({ name: "empty.csv", size: 0 }));
...
test("initCsvUpload toggles process button disabled state from file validation", () => {
  ...
  input.files = [buildCsvFile({ name: "people.csv", size: 4 })];
  onChange();

  assert.equal(processButton.disabled, false);
  assert.equal(status.dataset.state, "success");

  input.files = [buildCsvFile({ name: "empty.csv", size: 0 })];
  onChange();
